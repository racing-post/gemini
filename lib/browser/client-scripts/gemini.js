...
function prepareScreenshotUnsafe(selectors, opts) {
    resetZoom();
    var rect = getCaptureRect(selectors, opts.xpath);
    if (rect.error) {
        return rect;
    }
    ...
    return {
        ...
        ignoreAreas: findIgnoreAreas(opts.ignoreSelectors, opts.xpath),
...
function resetZoom() {
    ...
    meta.content = 'width=device-width,inital-scale=1.0,user-scalable=no';
}

function getCaptureRect(selectors, xpath) {
    var element, elementRect, rect;
    for (var i = 0; i < selectors.length; i++) {
        element = xpath ? query.xpath(selectors[i]) : query.first(selectors[i]);
        console.log(element);
        if (!element) {
            return {
                error: 'NOTFOUND',
...
function findIgnoreAreas(selectors, xpath) {
    var result = [];
    util.each(selectors, function(selector) {
        var element = xpath ? query.xpath(selector) : query.first(selector);
        if (element) {
